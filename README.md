...
## Testing
Backend smoke tests (use in-memory test data and do not require a running database):
```bash
cd backend
pip install -r requirements.txt
pytest tests
...
npm run build
```
Running the full stack via Docker (below) is recommended when you need to exercise frontend ↔ backend integration over HTTPS.
